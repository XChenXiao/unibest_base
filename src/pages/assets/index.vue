    ...
    if (res.status === 'success') {
      ...
      if (apiResponse && apiResponse.data) {
        ...
        if (responseData.registration && responseData.registration.length > 0) {
          ...
          // 字段映射：has_claimed=是否已领取，has_claimable=是否可领取
          ...
          // 保存交易ID用于领取奖励
          equityInfo.registrationTransactionId = registrationReward.claimable_transaction_id
        ...
        if (responseData.invitation && responseData.invitation.length > 0) {
          // 找到需要邀请人数最少的奖励
          const sortedInvitations = [...responseData.invitation].sort(
            (a, b) => a.invite_count - b.invite_count,
          )
          ...
          // 保存交易ID
          equityInfo.invitationTransactionId = firstInvitation.claimable_transaction_id
